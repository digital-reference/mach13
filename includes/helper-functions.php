...
EOT;
    ...
    function mf_send_notification($dbh,$form_id,$entry_id,$to_emails,$email_param){
    	...
    	if(!empty($email_address)){
			...
			$s_headers->addTextHeader('X-MachForm-Origin', $_SERVER['HTTP_HOST'].' '.$form_id);
...
EOT;
    ...
    function mf_get_template_variables($dbh,$form_id,$entry_id,$options=array()){
		...
		$entry_options['hide_encrypted_data']	 = $options['hide_encrypted_data'];
		$entry_options['hide_password_data']	 = $options['hide_password_data'];
		$entry_options['encryption_private_key'] = $options['encryption_private_key'];
			...
			foreach ($entry_details as $data){
				...
				if($data['element_type'] == 'file'){
					...
					if(!empty($options['machform_path'])){ //if the form is called from advanced form code
						...
						$data['value'] = '<img height="'.$signature_height.'" style="max-width: 430px" src="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']).'/signature_img.php?q='.$q_string.'" />';
					}
	...
	function mf_approval_sign($dbh,$form_id,$entry_id,$signature_params){
		...
		//get form properties, to check if payment enabled or not
    	$form_properties 	  = mf_get_form_properties($dbh,$form_id,array('logic_email_enable','logic_webhook_enable'));
    ...
    function mf_save_entry_to_google_sheet($dbh,$form_id,$entry_id){
		...
		$sheet_range = 'Sheet1';
//echo "<pre>";var_dump($request_params);var_dump($request_body);die("stop");
		$google_service->spreadsheets_values->append($spreadsheet_id, $sheet_range, $request_body, $request_params);
    ...
    //re-count the content of ap_form_stats table
    //if $user_id is empty, it apply to all forms
    function mf_refresh_form_stats($dbh,$user_id=0){
    	
    	//get the form id array
    	if(!empty($user_id)){
    		$query = "SELECT * FROM (SELECT 
										   A.form_id,
										   (select count(*) from `".MF_TABLE_PREFIX."permissions` where user_id=? and A.form_id=form_id) has_permission    
									   FROM 
		...
		if(!empty($form_id_array)){
			...
			foreach ($form_id_array as $form_id) {
				...
				$form_stats[$i]['total_entries'] = $row['total_entries'];

				//get today entry
				$current_date = date("Y-m-d").' 00:00:00';
				$query = "select count(*) today_entries from `".MF_TABLE_PREFIX."form_{$form_id}` where `status`=1 and date_created >= '{$current_date}' ";
				$sth = mf_do_query($query,array(),$dbh);
				$row = mf_do_fetch_result($sth);
				
				$form_stats[$i]['today_entries'] = $row['today_entries'];
				...
				$form_stats[$i]['last_entry_date'] = $row['date_created'];
