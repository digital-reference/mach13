	...
	function mf_process_form($dbh,$input){
		...
		foreach ($user_input as $element_name=>$element_data){
			...
			if('text' == $element_type){ //Single Line Text
				...
				$validation_result = mf_validate_element($flag,$element_type,$target_input,$rules);
				...
				if(!$is_matrix_field && !empty($element_info[$element_id]['choice_max_entry']) && $validation_result === true 
				...
				if(!file_exists($listfile_name) && MF_STORE_FILES_AS_BLOB === false){
