 ...
 ********************************************************************************/
			...
			if(!empty($form_encryption_keypair) && !empty($element['element_is_encrypted'])){
				try{
					if($element_type == 'simple_name'){
						$entry_data['element_'.$element_id.'_1'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_1']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_2'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_2']),$form_encryption_keypair);
					}else if($element_type == 'name'){
						$entry_data['element_'.$element_id.'_1'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_1']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_2'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_2']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_3'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_3']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_4'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_4']),$form_encryption_keypair);
						...
						$entry_data['element_'.$element_id.'_3'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_3']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_4'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_4']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_5'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_5']),$form_encryption_keypair);
						...
						$entry_data['element_'.$element_id.'_4'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_4']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_5'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_5']),$form_encryption_keypair);
						$entry_data['element_'.$element_id.'_6'] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id.'_6']),$form_encryption_keypair);
					}else{
						//for other element types (single line text, paragraph, email, phone, simple_phone)
						$entry_data['element_'.$element_id] = \Sodium\crypto_box_seal_open(base64_decode($entry_data['element_'.$element_id]),$form_encryption_keypair);
					}
				}catch(Error $e){
					//discard any error, so that we can proceed gracefully
				}catch(Exception $e){
	...
	function mf_get_entry_details($dbh,$form_id,$entry_id,$options=array()){
		...
		$hide_password_data  = $options['hide_password_data']; //if set to 'true' the data of the password field will be masked with asterisk
		...
		foreach ($form_elements as $element){
			...
			if('simple_name' == $element_type){ //Simple Name - 2 elements
					...
					foreach ($checkbox_childs as $option_id=>$option_label){
						if(!empty($entry_data['element_'.$element_id.'_'.$option_id])){
							if(empty($options['strip_checkbox_image'])){
								$checkbox_content .= '<img src="'.$checkbox_image.'" align="absmiddle" alt="" /> '.$option_label.'<br />';
							}else{
				...
				if(!empty($entry_data['element_'.$element_id.'_other'])){
					...
					if(empty($options['strip_checkbox_image'])){
						$checkbox_content .= '<img src="'.$checkbox_image.'" align="absmiddle" alt="" /> '.$entry_data['element_'.$element_id.'_other'];
					}else{
				...
				if(!empty($filename_array)){
					...
					foreach ($filename_array as $filename_value){
						...
						if(!empty($options['show_attach_image'])){
							...
							$entry_details[$i]['value'] .= '<img src="'.$options['machform_path'].'images/icons/185.png" align="absmiddle" style="vertical-align: middle" alt="" />&nbsp;'."{$filename_value} {$file_size}<br/>";
						}else{
							$filename_value = htmlspecialchars($filename_value);
							...
							if(!empty($options['strip_download_link'])){
								...
								if($show_image_preview === true && $is_image_file){
									if(!empty($options['machform_path'])){ //if the form is called from advanced form code
										$entry_details[$i]['value'] .= '<img src="'.$options['machform_path'].'view_img.php?q='.$q_string.'" style="max-width: 350px;margin: 15px 0;" alt="" /><br/><a class="entry_link" href="'.$options['machform_path'].'download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}else{
										$entry_details[$i]['value'] .= '<img src="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].rtrim(dirname($_SERVER['PHP_SELF']),'/').'/view_img.php?q='.$q_string.'" style="max-width: 350px;margin: 15px 0;" alt="" /><br/><a class="entry_link" href="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].rtrim(dirname($_SERVER['PHP_SELF']),'/').'/download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}
								}else{
									if(!empty($options['machform_path'])){ //if the form is called from advanced form code
										$entry_details[$i]['value'] .= '<a class="entry_link" href="'.$options['machform_path'].'download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}else{
										$entry_details[$i]['value'] .= '<a class="entry_link" href="http'.$ssl_suffix.'://'.$_SERVER['HTTP_HOST'].rtrim(dirname($_SERVER['PHP_SELF']),'/').'/download.php?q='.$q_string.'">'.$filename_value.'</a><br/>';
									}
								}
							}
	...
	function mf_display_entries_table($dbh,$form_id,$options){
		...
		$display_approval_status_when_enabled 	= $options['display_approval_status_when_enabled'];
			...
			foreach($column_prefs as $column_name){
				...
				if($column_type_lookup[$column_name] == 'time'){
					...
					if($matrix_type == 'radio'){
						...
						if(!empty($row[$column_name])){
							$form_data[$i][$j]  = '<div class="me_center_div"><img src="images/icons/62_blue_16.png" align="absmiddle" alt=""/></div>';
						}else{
							$form_data[$i][$j]  = '';
						}
					}
